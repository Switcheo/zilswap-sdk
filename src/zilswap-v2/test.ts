...
const test = async () => {
  ...
  try {
    ...
    // swap
    // const r1 = await zilswap.getOutputForExactInput(swthHash, hunyHash, new BigNumber(amountIn).toString())
    ...
    // const r2 = await zilswap.getInputForExactOutput(swthHash, hunyHash, new BigNumber(amountInMax).toString())
    ...
    // const r3 = await zilswap.getOutputForExactInput(wZilHash, hunyHash, new BigNumber(amountIn).toString())
    // console.log(`\n${amountIn} ZIL -> SWTH: ${r3} SWTH\n`)
    tx = await zilswap.swapExactZILForTokens([pool2], wZilHash, new BigNumber(amountIn).toString(), new BigNumber(amountOutMin).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()
    tx = await zilswap.swapExactZILForTokens([pool2, pool1], wZilHash, new BigNumber(amountIn).toString(), new BigNumber(amountOutMin).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()

    // const r4 = await zilswap.getInputForExactOutput(wZilHash, swthHash, new BigNumber(amountInMax).toString())
    // console.log(`\nZIL -> ${amountOut} SWTH: ${r4} ZIL\n`)
    tx = await zilswap.swapZILForExactTokens([pool3], wZilHash, new BigNumber(amountInMax).toString(), new BigNumber(amountOut).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()
    tx = await zilswap.swapZILForExactTokens([pool3, pool1], wZilHash, new BigNumber(amountInMax).toString(), new BigNumber(amountOut).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()

    // const r5 = await zilswap.getOutputForExactInput(hunyHash, wZilHash, new BigNumber(amountIn).toString())
    // console.log(`\n${amountIn} HUNY -> ZIL: ${r5} HUNY\n`)
    tx = await zilswap.swapExactTokensForZIL([pool3], hunyHash, new BigNumber(amountIn).toString(), new BigNumber(amountOutMin).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()
    tx = await zilswap.swapExactTokensForZIL([pool1, pool2], hunyHash, new BigNumber(amountIn).toString(), new BigNumber(amountOutMin).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()

    // const r6 = await zilswap.getInputForExactOutput(hunyHash, wZilHash, new BigNumber(amountInMax).toString())
    // console.log(`\nZIL -> ${amountOut} SWTH: ${r6} ZIL\n`)
    tx = await zilswap.swapTokensForExactZIL([pool2], swthHash, new BigNumber(amountInMax).toString(), new BigNumber(amountOut).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()
    tx = await zilswap.swapTokensForExactZIL([pool1, pool3], swthHash, new BigNumber(amountInMax).toString(), new BigNumber(amountOut).toString())
    console.log(`\ntx hash: ${tx.hash}\n`)
    await waitForTx()
